...
describe("handleExecTransaction tests", () => {
  ...
  test("Should handle normal nonce call", () => {
    let normalSafe = Address.fromString(
      "0xF5DeC41B8597Bded2706a8D57FE011d25A138d6A"
    );
    createMockedFunction(normalSafe, "nonce", "nonce():(uint256)").returns([
      ethereum.Value.fromUnsignedBigInt(BigInt.fromI32(1)),
    ]);
    createMockedFunction(normalSafe, "getTransactionHash", "getTransactionHash(address,uint256,bytes,uint8,uint256,uint256,uint256,address,address,uint256):(bytes32)").withArgs(
      [
