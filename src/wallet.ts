...
export function handleTransaction(walletAddr: Address, hash: Bytes, to: Address, value: BigInt, data: Bytes, operation: i32, safeTxGas: BigInt, baseGas: BigInt, gasPrice: BigInt,  gasToken: Address, refundReceiver: Address, signatures: Bytes): void {
    ...
    if(wallet != null) {
        ...
        if (!currentNonce.reverted) {
          let nonce = currentNonce.value.equals(zeroBigInt()) ? currentNonce.value : currentNonce.value.minus(oneBigInt())
        ...
        log.warning("handleTransaction::Wallet: {} transaction {} - cannot get nonce (reverted)", [walletAddr.toHexString(), hash.toHexString()])
      }
    } else {
